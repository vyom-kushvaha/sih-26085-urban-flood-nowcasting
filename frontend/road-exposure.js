  ...
  try{
    ...
    L.geoJSON(data,{pane:'roadExposure',renderer:state.roadRenderer,style:f=>({color:f.properties.color,weight:state.map.getZoom()<=11?2.5:4,opacity:.75}),
      onEachFeature:(feature,layer)=>{const p=feature.properties,depth=p.estimated_1h_depth_range_cm;
        ...
        layer.bindPopup(`<b>${esc(p.name)} · ${esc(p.category)}</b><br>Basis: ${esc(p.classification_basis)}<br>Live rainfall: ${esc(p.rainfall_mm_hr??'Unavailable')} mm/hr<br>Verified observed water: ${observed==null?'Unavailable':esc(observed)+' cm'}<br>1h modeled depth: ${depth?esc(depth[0])+'–'+esc(depth[1])+' cm':'Unavailable'}<br>Safety certification: No`);}
    ...
    $('updated').textContent=`Weather ${m.weather_coverage_pct}% · Verified depth ${m.verified_observation_road_count||0}`;
