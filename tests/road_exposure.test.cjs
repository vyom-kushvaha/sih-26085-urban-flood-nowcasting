...
test('visible road overlay reports coverage and preserves unverified status',async()=>{
  ...
  const data={type:'FeatureCollection',features:[{type:'Feature',geometry:{type:'LineString',coordinates:[[72.84,19],[72.85,19.01]]},properties:{name:'Test Road',category:'BLUE',color:'#2563eb',classification_basis:'LIVE_RAINFALL_SCREENING',rainfall_mm_hr:2,weather_source:'MET Norway Locationforecast numerical forecast',safe_route_certified:false}}],metadata:{road_count:1,weather_coverage_pct:100,weather_sources:['MET Norway Locationforecast numerical forecast'],terrain_model_coverage_pct:0,verified_observation_road_count:0,category_counts:{RED:0,ORANGE:0}}};
  ...
  assert.match(get('updated').textContent,/Weather 100% · MET Norway/);
